...
import { fetchSynopsis } from "@/lib/APIservice";
...
export default function RFPSynopsis() {
  ...
  const { data, isLoading, error } = useQuery({
    queryKey: ["rfpSynopsis"],
    queryFn: fetchSynopsis,
  });
    ...
    <div className="flex h-screen bg-gray-100 relative overflow-hidden">
      ...
      <div className="flex-1 overflow-auto w-full">
        <div className="mx-auto bg-white min-h-full shadow-sm">
          <Header title='RFP Synopsis' setSidebarOpen={setSidebarOpen} sidebarOpen={sidebarOpen} />
          <div className="p-6 space-y-6">
            <h1 className="text-2xl font-semibold">RFP Synopsis</h1>

            {isLoading ? (
              <div className="text-center text-gray-500">Loading synopsis...</div>
            ) : error ? (
              <div className="text-center text-red-500">
                Failed to load synopsis: {(error as Error).message}
              </div>
            ) : !data?.result ? (
              <div className="text-center text-gray-500">No synopsis data available</div>
            ) : (
